...
Status: proposed transport contract with the T01 foundation and T05 Classification routes implemented. Other financial routes remain proposals and are not evidence of implemented endpoints.

Implementation note: `Database.api_write_unit_of_work()` and `IdempotencyStore` provide the API-owned transaction participation and response-recording primitive described below. Existing service calls made through `Database.session()` and `Database.write_session()` join that transaction when invoked inside it. FastAPI lifecycle, `/api/v1/health`, `/api/v1/auth/session` (POST/GET/DELETE), and the authenticated T05 `/api/v1/classification` router are implemented. Classification mutations are conditional on the currently loaded state, but are not idempotent or declared safe for automatic retry after a timeout; other financial routes remain proposals and must pass route-specific review.
...
Classification additionally returns `409 STALE_CLASSIFICATION` when an override version changed since queue load and `409 STALE_RULE` when the current exact-match rule revision changed since preview/history load.
...
T05 implements conditional checks for classification writes inside `Database.api_write_unit_of_work()`. The API router acquires `BEGIN IMMEDIATE` before reading the current version and keeps the lock through the service write, so concurrent API writers cannot both pass the same stale precondition. An override version is the greatest ID among the latest per-field override rows for that transaction, including clear tombstones; it is 0 before any override history. Queue items also include an opaque SHA-256 `expected_classification_state` token computed from that effective `ClassificationResult` and override version, so a changed source row or rule is detected too. Override POST/DELETE must present both queue values. A rule precondition is the current rule ID for the exact account-kind/direction/source-category/source-movement/currency key; null means no revision existed when the preview ran. Rule creation must present the preview's current ID, and disable must present the current active ID. A mismatch returns 409 and writes nothing. These checks protect callers of these API routes; legacy CLI/Streamlit calls do not carry API preconditions and retain their existing append/last-writer semantics. Classification writes are still not idempotent: after a timeout the client shows outcome unknown, reloads state, and requires an explicit user decision before resubmitting.

`GET /api/v1/classification/coverage` calls `ClassificationService.classify_many()` for accepted transactions and returns classification counts by class/source, review-required count, issue counts, and the classified share. This is classification coverage of accepted rows only. It does not establish source-period coverage, account completeness, or household-wide data completeness; those remain Data Quality responsibilities.
...
| Classification review and coverage | GET /classification/review-queue; GET /classification/coverage; POST /classification/transactions/{transaction_id}/override; DELETE /classification/transactions/{transaction_id}/override | ClassificationService.review_queue, classify_many, save_override, clear_override |
...
| Classification override | POST requires `expected_override_version`, `expected_classification_state`, at least one of economic_class, analysis_category, expense_behavior, and reason. Queue items return the current version (0 when no override history exists) and the effective-state hash. DELETE requires both preconditions; `fields` and reason are optional. |
| Reusable classification rule | account_kind, direction, source_category, currency, economic_class; optional source_movement_type, analysis_category, expense_behavior, reason. Preview uses the same fields, returns the exact-match count and `expected_current_rule_id` (nullable). Create must send that value as a required nullable `expected_current_rule_id`; disable sends the current id in the same named field. |
