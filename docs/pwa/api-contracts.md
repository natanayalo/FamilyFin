...
Status: transport contract with the T01 foundation and T09 Net Worth adapter implemented. FastAPI health, authentication/session, and net-worth endpoints are registered; remaining feature routes below are still proposals.

Implementation note (T01 foundation): `Database.api_write_unit_of_work()` and `IdempotencyStore` provide the API-owned transaction participation and response-recording primitive described below. Existing service calls made through `Database.session()` and `Database.write_session()` join that transaction when invoked inside it. FastAPI lifecycle, `/api/v1/health`, and `/api/v1/auth/session` (POST/GET/DELETE) are implemented. T01 registered no financial routes. Each feature route must use the authenticated router and pass route-specific atomicity/external-effect review before advertising replay safety.

Implementation note (T09 Net Worth): authenticated `/api/v1/net-worth` routes adapt the existing `NetWorthService` for account registry/lifecycle, observed snapshots and immutable revisions, strict CSV template/preview/commit, account history, summary, trend, and forecast comparison. Decimal response values are strings and financial responses are no-store. Account reads return `updated_at`; account edit, close, and reactivate requests must send that exact value as `expected_updated_at`. The API re-reads it inside the API-owned write transaction and returns 409 `STALE_ACCOUNT` when another user has changed the account. Account and snapshot mutations write actor audit events in the same database transaction as their financial changes. CSV commit also writes its actor event with its financial database records, but installs the content-addressed archive file outside SQLite; the file and database are not atomic. Net-worth mutations do not advertise replay safety or send automatic retries. On a CSV timeout the result is outcome-unknown; reconcile the import state before deciding whether to submit again. The forecast comparison API accepts exact forecast and observed revision identities; its PWA chooser/presentation is deferred until the Savings Forecast read contract is implemented.
...
Current service writes without a comparable service-level precondition include classification override/rule operations and scenario/forecast/study archive toggles. Net-worth account edits, closes, and reactivations use an API-owned `expected_updated_at` check: the API starts the serialized write transaction, re-reads the account, and compares the exact UTC timestamp before invoking the service mutation. A mismatch returns 409 `STALE_ACCOUNT`; clients should reload before deciding whether to reapply an edit.
...
| Net-worth CSV/revisions/reporting | GET /net-worth/csv-template; POST /net-worth/csv/previews; POST /net-worth/csv/commits; GET /net-worth/snapshots/{snapshot_id}/revisions[/{revision_number}]; POST /net-worth/snapshots/{snapshot_id}/revisions/{revision_number}/restore; POST /net-worth/snapshots/{snapshot_id}/archive (archived true/false); GET /net-worth/accounts/{account_key}/history; GET /net-worth/summary; GET /net-worth/trend; POST /net-worth/forecast-comparisons | NetWorthService.csv_template, preview_csv, commit_csv, list_revisions/get_revision, restore_revision, archive_snapshot/unarchive_snapshot, account_history, summary, trend, compare_forecast_actual. Implemented by T09; the forecast comparison UI awaits the Savings Forecast read API. |
