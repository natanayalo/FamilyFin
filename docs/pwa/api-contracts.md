...
Implementation note (T09 Net Worth): authenticated `/api/v1/net-worth` routes now adapt the existing `NetWorthService` for account registry/lifecycle, observed snapshots and immutable revisions, strict CSV template/preview/commit, account history, summary, trend, and forecast comparison. Decimal response values are strings and financial responses are no-store. Net-worth mutations do not advertise replay safety or send automatic retries: CSV commit installs an archive file as well as database state, and the other mutations have no stored idempotency response. Account lifecycle writes also retain the service's documented lack of an optimistic revision precondition. The forecast comparison API accepts exact forecast and observed revision identities; its PWA chooser/presentation is deferred until the Savings Forecast read contract is implemented.
