...
Implementation note: `Database.api_write_unit_of_work()` and `IdempotencyStore` provide the API-owned transaction participation and response-recording primitive described below. Existing service calls made through `Database.session()` and `Database.write_session()` join that transaction when invoked inside it. FastAPI lifecycle, `/api/v1/health`, and `/api/v1/auth/session` (POST/GET/DELETE) are implemented. T04 registers authenticated quality, import, and reconciliation routes. Reconciliation decisions and their actor audit events commit with the financial mutation and idempotency response. FamilyBiz commits write the actor audit event with the financial database transaction, while their XLSX archive write remains a separate filesystem effect; a timeout is outcome-unknown and does not make a retry safe.
...
This gives a defined crash outcome: before commit, the financial write, actor audit event, and idempotency record roll back; after commit, all three exist and a retry returns the recorded response without adding a second event. A focused integration test exercises a PlanningService write through the API-owned boundary, injects a failure before record/commit, and verifies rollback followed by successful retry/replay. T04 reconciliation resolution uses this boundary and is replay-safe. FamilyBiz import commit also writes its actor audit event inside the same SQLite transaction as its financial rows, but its XLSX archive write occurs in the filesystem and cannot participate in that transaction. A database rollback may therefore leave an orphan archive file, and a committed database transaction does not make a timed-out HTTP response safely replayable. FamilyBiz commits remain outcome-unknown after a timeout until an independently verified archive recovery/deduplication mechanism exists.
