...
Uvicorn access logging is disabled. The API creates server request IDs and returns them in the standard error envelope and `X-Request-ID`; application logs must not include bodies, cookies, authorization values, filenames, descriptions, amounts, account references, client addresses, or usernames. Append-only `actor_audit_events` records only account ID, request ID, short event/target codes, outcome, and UTC time. Net-worth account and snapshot routes write actor events in the same API-owned database transaction as their financial changes. CSV commit does the same for its financial database rows, while its content-addressed archive remains a separate filesystem effect; neither audit nor financial rows make that archive atomic. These mutations do not claim replay safety. If a CSV commit times out, the browser shows outcome unknown and the user reconciles import state before deciding whether to submit again.
