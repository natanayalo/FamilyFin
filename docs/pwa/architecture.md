...
Implementation update: T01 provides a FastAPI app, two local household identities, server-side sessions, CSRF/origin controls, request limits, request IDs, safe actor audit metadata, and atomic idempotency primitives. T04 adds Data Quality, FamilyBiz import, history, and reconciliation routes. Reconciliation decisions record the actor in the same API-owned transaction as the decision and idempotency response. FamilyBiz commits record the actor with their financial database writes, while the archive remains a separate filesystem effect; a timed-out response remains outcome-unknown pending verified archive recovery. Data-quality dashboard facades are request-scoped because the metrics service has mutable per-series context. See [deployment.md](deployment.md) and [api-contracts.md](api-contracts.md).
...
Domain provenance records imported source files, batches, transaction links, assumptions, and immutable revisions. The API also writes append-only actor audit records for authenticated reconciliation decisions and FamilyBiz commits. Those records join the same SQLite transaction as their financial writes; reconciliation also stores its replay response there. FamilyBiz archive files remain outside SQLite atomicity, so a database rollback can leave an archive file and a lost response remains outcome-unknown. Streamlit/CLI actor attribution remains a separate follow-up because those flows do not carry authenticated API identities.
